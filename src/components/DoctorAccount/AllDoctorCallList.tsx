...
const AllDoctorCallList = () => {
  const [loading, setLoading] = useState(false);
  const [allCallHistory, setAllCallHistory] = useState<callInterface[]>();
    ...
    try {
      ...
      if (doc_Id && userAuthentication) {
        ...
        if (data.role === "inhouse_doctor") {
          const { data } = await apiCall(
            Methods.GET,
            `${EndPoint.CALL_HISTORY_GET}?page=${activePage}&startDate=${startTime}&endDate=${startTime}`
          );
          // console.log("First Time Init ");
          console.log(data);
          setAllCallHistory(data.callHistory);
          setTotal(data.count);
        } else {
          toast.error("Sorry, You Are Not Inhouse Doctor");
        }
      }
  ...
  const handleChange = async (value: [Date, Date] | null) => {
    try {
      ...
      if (value) {
        ...
        const { data } = await apiCall(
          Methods.GET,
          `${EndPoint.CALL_HISTORY_GET}?startDate=${startTime}&endDate=${endTime}`
        );
        // console.log("After Changed time", data);
        ...
        const { data } = await apiCall(
          Methods.GET,
          `${EndPoint.CALL_HISTORY_GET}?startDate=${startTime}&endDate=${startTime}`
        );
        setFirstDate(startTime);
        setLastDate(startTime);
        // console.log("After Rrmove Time", data);
  ...
  const onPageSearch = async (page: number) => {
    ...
    try {
      ...
      const { data } = await apiCall(
        Methods.GET,
        `${EndPoint.CALL_HISTORY_GET}?startDate=${firstDate}&endDate=${lastDate}&page=${page}`
      );
      setAllCallHistory(data.callHistory);
      setTotal(data.count);
      // console.log("On Page Search");
      console.log(data);
    } catch (error) {
    ...
    try {
      ...
      if (response.success) {
        const response = await apiCall(
          ...
          }?startDate=${startTime}&endDate=${startTime}&page=${1}`
        );
        setAllCallHistory(response.data.callHistory);
...
export default AllDoctorCallList;
