...
export const POST = async (req: Request) => {
  ...
  if (decision === "CANCEL") {
    const value = {
      ...
      auth_trans_ref_no: object.req_reference_number,
    ...
    if (response.success == true) {
      ...
      // );
      redirect(
        `${process.env.NEXT_PUBLIC_FRONTEND_URL}/en/mobilecancel?auth_time=${auth_time}&req_reference_number=${req_reference_number}&auth_amount=${auth_amount}&firstName=${firstName}&lastName=${lastName}&address_line1=${req_bill_to_address_line1}&address_city=${req_bill_to_address_city}&country=${country}&phone_no=${phone_no}&email=${email}&card_type=${card_type}&card_number=${card_number}&auth_code=${auth_code}&auth_reconciliation_reference_number=${auth_reconciliation_reference_number}`
      );
    } else {
      redirect(`${process.env.NEXT_PUBLIC_FRONTEND_URL}/en/mobilecancel?from=2`);
    }
  } else {
    redirect(`${process.env.NEXT_PUBLIC_FRONTEND_URL}/en/mobilecancel?From=3`);
  }
};
